...
private:
    ...
    void initVulkan() {
        ...
        physicalDevice.pick(instance, msaaSamples, surface, deviceExtensions);
        device.setUp(physicalDevice.get(), surface.get(), deviceExtensions, graphicsQueue, presentQueue);
        swapChain.create(physicalDevice.get(), device.get(), surface, window);
        createImageViews();
        createRenderPass();
    ...
    void cleanupSwapChain() {
        ...
        vkFreeCommandBuffers(device.get(), commandPool, static_cast<uint32_t>(commandBuffers.size()), commandBuffers.data());
    ...
    void cleanup() {
        ...
        if (enableValidationLayers) {
            debugMessenger.DestroyDebugUtilsMessengerEXT(instance.get(), nullptr);
        }
   ...
   //k void createSwapChain() {
   //k     SwapChainSupportDetails swapChainSupport = querySwapChainSupport(physicalDevice.get(), surface.get());

   //k     VkSurfaceFormatKHR surfaceFormat = chooseSwapSurfaceFormat(swapChainSupport.formats);
   //k     VkPresentModeKHR presentMode = chooseSwapPresentMode(swapChainSupport.presentModes);
   //k     VkExtent2D extent = chooseSwapExtent(swapChainSupport.capabilities);

   //k     uint32_t imageCount = swapChainSupport.capabilities.minImageCount + 1;
   //k     if (swapChainSupport.capabilities.maxImageCount > 0 && imageCount > swapChainSupport.capabilities.maxImageCount) {
   //k         imageCount = swapChainSupport.capabilities.maxImageCount;
   //k     }

   //k     VkSwapchainCreateInfoKHR createInfo{};
   //k     createInfo.sType = VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR;
   //k     createInfo.surface = surface.get();

   //k     createInfo.minImageCount = imageCount;
   //k     createInfo.imageFormat = surfaceFormat.format;
   //k     createInfo.imageColorSpace = surfaceFormat.colorSpace;
   //k     createInfo.imageExtent = extent;
   //k     createInfo.imageArrayLayers = 1;
   //k     createInfo.imageUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;

   //k     QueueFamilyIndices indices = findQueueFamilies(physicalDevice.get(), surface.get());
   //k     uint32_t queueFamilyIndices[] = {indices.graphicsFamily.value(), indices.presentFamily.value()};

   //k     if (indices.graphicsFamily != indices.presentFamily) {
   //k         createInfo.imageSharingMode = VK_SHARING_MODE_CONCURRENT;
   //k         createInfo.queueFamilyIndexCount = 2;
   //k         createInfo.pQueueFamilyIndices = queueFamilyIndices;
   //k     } else {
   //k         createInfo.imageSharingMode = VK_SHARING_MODE_EXCLUSIVE;
   //k     }

   //k     createInfo.preTransform = swapChainSupport.capabilities.currentTransform;
   //k     createInfo.compositeAlpha = VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR;
   //k     createInfo.presentMode = presentMode;
   //k     createInfo.clipped = VK_TRUE;

   //k     if (vkCreateSwapchainKHR(device.get(), &createInfo, nullptr, &swapChain) != VK_SUCCESS) {
   //k         throw std::runtime_error("failed to create swap chain!");
   //k     }

   //k     vkGetSwapchainImagesKHR(device.get(), swapChain, &imageCount, nullptr);
   //k     swapChainImages.resize(imageCount);
   //k     vkGetSwapchainImagesKHR(device.get(), swapChain, &imageCount, swapChainImages.data());

   //k     swapChainImageFormat = surfaceFormat.format;
   //k     swapChainExtent = extent;
   //k }
    ...
    void createColorResources() {
        ...
        createImage(swapChain.getExtent().width, swapChain.getExtent().height, 1, msaaSamples, colorFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, colorImage, colorImageMemory);
    ...
    void createDepthResources() {
        ...
        createImage(swapChain.getExtent().width, swapChain.getExtent().height, 1, msaaSamples, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMemory);
    ...
    void generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels) {
        ...
        vkGetPhysicalDeviceFormatProperties(physicalDevice.get(), imageFormat, &formatProperties);
    ...
    void createUniformBuffers() {
        ...
        uniformBuffersMemory.resize(swapChain.getImages().size());
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        void* data;
        vkMapMemory(device.get(), uniformBuffersMemory[currentImage], 0, sizeof(ubo), 0, &data);
            memcpy(data, &ubo, sizeof(ubo));
        vkUnmapMemory(device.get(), uniformBuffersMemory[currentImage]);
    }
